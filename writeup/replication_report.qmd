...
## Import data
data_searcher = read.csv('../data/pilot_a/searcher_behavior.csv') # model searchers from the original paper
data_human    = read.csv('../data/pilot_a/human_behavior.csv')    # human searcher (n=1)
data_eye      = read.csv('../data/pilot_a/eye_track.csv')         # human eye fixation time-series data
...
for (i_trial in 1:nrow(data_human)){
  # number of fixations = 1 + fixation onsets between stimulus onset & target detection
  n_fix[i_trial] = 1 + sum(
...
data_compare = data_searcher %>% filter(searcher=='random')
...
**Searcher model simulation**. Searcher model predictions are yet to be fully implemented. For the pilot A, the read-out values from the original paper were used for the following model comparison.
...
```{r}
null = lm(n_fixation ~ visibility, data=data_compare)
full = lm(n_fixation ~ visibility*searcher, data=data_compare)
...
Here, I observed that the full model led to a significantly improved fit over the null model, implying that my pilot data differs the random searcher behavior. An effect size will be calculated afterwards, with the mixed effects across paricipants considered.

**Figure**. When I side-by-side compared the relationship between number of fixations and visibility, a pattern was observed: the behavioral fixations performed better than the ideal model, which is contradictory to the original paper. I suspect that this is due to the poor measurement of the [visibility](#visibility), underestimating the measurements on the abscissa. I plan to work more on the reliable measurement of visibility.

```{r}
data_human_compare %>% 
  ggplot(aes(x=visibility, y=n_fixation)) +
  stat_summary(color='blue', fun.data=mean_se) + 
  geom_line(data=data_searcher, aes(linetype=searcher), color='blue') + 
  coord_cartesian(xlim=c(3,7), ylim=c(0,20)) +
  xlab("Foveal target visibility (d', estimated)") + ylab("Number of fixations") +
  theme_classic()
```
